...
<Note>
  Fields like `calories_kcal`, `distance_meters`, and heart-rate fields are `null` when the provider did not report them. Hevy strength workouts include the complete exercise, set, repetition, load, RPE, distance, and duration data in `exercises`, making the event self-contained for coaching agents.
</Note>
