...
The current Apple SDK supplies authoritative whole-workout distance and elapsed time. A run is eligible for a standard-distance record only when its completed distance is close to that standard distance; Open Wearables does not extrapolate a 5K result from a longer run's average pace. Records identify this provenance with `calculation_method=whole_run` and `confidence=high`.

Fastest continuous segments inside a longer run require timestamped distance, speed, or route samples. Until those samples are supplied by the mobile bridge, in-run segment PRs are intentionally omitted rather than estimated.
